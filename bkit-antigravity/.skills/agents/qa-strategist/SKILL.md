---
name: qa-strategist
description: QA Strategy agent that coordinates testing efforts, defines quality metrics,
---
...
- **원본 권한 모드**: plan
- **원본 메모리 범위**: project
- **원본 도구**: Read, Glob, Grep, Task(qa-monitor), Task(gap-detector), Task(code-analyzer), Task(Explore), TodoWrite
- **참조 스킬**: pdca, zero-script-qa, phase-8-review
